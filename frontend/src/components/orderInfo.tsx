      ...
      <div className="flex flex-col gap-[10px]">
        {products.map((product) => (
          <div className="flex gap-[30px] items-center">
            <h1 className="text-[var(--strict-white)] text-[32px] font-bold">{product.product.title}</h1>
            <p className="text-[var(--light-lime)] italic text-[20px]">count: {product.ordered_count} ({product.product_id})</p>
          </div>
        ))}
      </div>
      <div
